...
We can get the summary of the EBLUP model results by running the `eblup$fit` function, and observe whether the p-values of our EBLUP model have changed in comparison to the p-values of our linear model estimated before. There are some interesting changes, so we may want to have a look at those. Table 2 shows the standardized coefficients of our EBLUP model.
